...
if (!db.prepare("SELECT id FROM users WHERE is_admin=1").get()) {
  ...
  const id = db.prepare("INSERT INTO users (username,password,avatar,is_admin) VALUES (?,?,?,1)")
...
if (db.prepare("SELECT COUNT(*) as c FROM users WHERE is_admin=0").get().c === 0) {
  const demos = [
    { u:"StormKing",   av:"⚡", th:"ice",  s:{level:55,wins:51,eliminations:489,fe:223,assists:134,kd:4.6,diamonds:11200,gold:42000,beds:91,vaults:31,upgrades:198,dmg:224100,clutch:62}},
    { u:"ShadowBlade", av:"🗡️", th:"fire", s:{level:47,wins:38,eliminations:412,fe:189,assists:97, kd:3.8,diamonds:8420, gold:31200,beds:74,vaults:22,upgrades:145,dmg:187400,clutch:43}},
    { u:"NightReaper", av:"💀", th:"void", s:{level:39,wins:29,eliminations:334,fe:141,assists:78, kd:2.9,diamonds:6130, gold:24800,beds:58,vaults:18,upgrades:112,dmg:143200,clutch:31}},
    { u:"IronFang",    av:"🔱", th:"fire", s:{level:28,wins:17,eliminations:198,fe:89, assists:55, kd:1.9,diamonds:3800, gold:16400,beds:38,vaults:11,upgrades:76, dmg:92300, clutch:18}},
  ];
  demos.forEach(({u,av,th,s}) => {
    const hash = bcrypt.hashSync("demo123", 10);
    const uid = db.prepare("INSERT INTO users (username,password,avatar,theme) VALUES (?,?,?,?)")
      .run(u, hash, av, th).lastInsertRowid;
    db.prepare(`INSERT INTO stats (user_id,level,wins,eliminations,final_eliminations,assists,kd,diamonds,gold,bed_destroyed,vaults_opened,upgrades,damage_dealt,clutch_eliminations) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
      .run(uid,s.level,s.wins,s.eliminations,s.fe,s.assists,s.kd,s.diamonds,s.gold,s.beds,s.vaults,s.upgrades,s.dmg,s.clutch);
    const pcts = [0.2,0.4,0.65,0.85];
    pcts.forEach((p,i) => {
      try { db.prepare("INSERT INTO progress (user_id,period,period_type,wins,eliminations,bed_destroyed) VALUES (?,?,?,?,?,?)")
        .run(uid,`2024-W0${i+1}`,"weekly",Math.floor(s.wins*p),Math.floor(s.eliminations*p),Math.floor(s.beds*p)); } catch(e){}
    });
    [0.45,1].forEach((p,i) => {
      try { db.prepare("INSERT INTO progress (user_id,period,period_type,wins,eliminations,bed_destroyed) VALUES (?,?,?,?,?,?)")
        .run(uid,`2024-M0${i+1}`,"monthly",Math.floor(s.wins*p),Math.floor(s.eliminations*p),Math.floor(s.beds*p)); } catch(e){}
    });
  });
}
...
const adminAuth = (req,res,next) => auth(req,res,()=>{ if(!req.user.isAdmin) return res.status(403).json({error:"Admin only"}); next(); });
...
function allPlayers() {
  return db.prepare(`
    ...
    WHERE u.is_admin=0 ORDER BY s.wins DESC
  `).all();
}
...
app.post("/api/register", (req,res) => {
  ...
  const uid = db.prepare("INSERT INTO users (username,password,avatar) VALUES (?,?,?)").run(username.trim(),hash,avatar||"🎮").lastInsertRowid;
  ...
  res.json({token,user:{id:uid,username,avatar:avatar||"🎮",theme:"fire",isAdmin:false}});
});

app.post("/api/login", (req,res) => {
  const {username,password} = req.body;
  const user = db.prepare("SELECT * FROM users WHERE username=?").get(username);
  if (!user||!bcrypt.compareSync(password,user.password)) return res.status(401).json({error:"Wrong username or password"});
...
app.get("/api/players", (_,res) => res.json(allPlayers()));

app.get("/api/players/:id/progress", (req,res) => {
  res.json(db.prepare("SELECT * FROM progress WHERE user_id=? ORDER BY period ASC").all(req.params.id));
});
...
app.put("/api/me/stats", auth, (req,res) => {
  ...
  [{ p:wk,t:"weekly"},{p:mo,t:"monthly"}].forEach(({p,t})=>{
    db.prepare(`INSERT INTO progress (user_id,period,period_type,wins,eliminations,bed_destroyed) VALUES (?,?,?,?,?,?)
      ...
      .run(req.user.id,p,t,wins||0,eliminations||0,bedDestroyed||0);
  });
  res.json({ok:true});
});
...
// Admin routes
app.get("/api/admin/players", adminAuth, (_,res) => res.json(allPlayers()));
...
app.post("/api/admin/players", adminAuth, (req,res) => {
  ...
  if (db.prepare("SELECT id FROM users WHERE username=?").get(username)) return res.status(409).json({error:"Username already taken"});
  const hash = bcrypt.hashSync(password,10);
  const uid = db.prepare("INSERT INTO users (username,password,avatar,theme) VALUES (?,?,?,?)").run(username.trim(),hash,avatar||"🎮",theme||"fire").lastInsertRowid;
  db.prepare("INSERT INTO stats (user_id) VALUES (?)").run(uid);
...
// ── Serve frontend (inline HTML — no React build needed) ──────────────────────
